...
import com.dreams.logistics.exception.MsgException;
...
@Slf4j
@Service
//设置初始化条件，只有在配置文件中设置dreams.mq.enable=true才能触发
@ConditionalOnProperty(prefix = "dreams.mq", value = "enable")
public class MQServiceImpl implements MQService {
        ...
        correlationData.getFuture().addCallback(
                result -> {
                    if (result.isAck()) {
                        ...
                        msgService.removeById(sendMsg);
                    } else {
                        // 3.2.nack，消息失败
                        log.error("消息发送失败, ID:{}, 原因:{}", correlationData.getId(), result.getReason());
                        FailMsg failMsg = FailMsg.builder()
                                .exchange(exchange)
                                .msgId(correlationData.getId())
                                .msg(jsonMsg)
                                .reason(result.getReason()).build();
                        this.failMsgService.save(failMsg);
                    }
                },
                ex -> {
    ...
    public Boolean saveFailMag(MsgException msgException) {
        ...
        this.failMsgService.save(msgException.getFailMsg());
        return true;
    }
